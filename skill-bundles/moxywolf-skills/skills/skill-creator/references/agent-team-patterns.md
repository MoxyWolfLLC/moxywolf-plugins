...
## Context economy: what each agent is allowed to know

The run-discipline section above is about what you can see afterwards. This one is about what each agent carries while it works — the other half of the same deer-flow lesson, and the half that decides both what a fan-out costs and whether its results are worth anything.

- **Load capability progressively, not preemptively.** A sub-agent gets the procedure for the job it was given, not the skill's whole reference shelf. Name the one or two reference files its task actually needs and say so in its prompt; let it read further only if the work turns out to require it. The failure this prevents is subtle: an agent handed six references spends its first thousand tokens deciding which one applies, and an agent whose context is 80% background it didn't need has correspondingly less room for the material it did. Progressive loading is a quality decision that happens to save money, not a cost optimization that happens to cost quality.
- **Isolate context deliberately when independence is the point.** Fan-out for *parallelism* wants shared context — the branches are doing one job faster. Fan-out for *independent perspectives* wants the opposite: each agent gets the artifact and the question and nothing else. No sibling results, no orchestrator framing, no hint about the expected answer. Three finders who can see each other's finds converge; three reviewers who can see the author's reasoning start defending it. Context isolation is the cheap approximation of a genuinely second opinion, and when a design says "independent," isolation is the thing that makes the word true.
- **Pass digests down, never raw result sets.** When one stage's output seeds the next, distill it first. A stage that inherits its parent's full findings re-derives them and reports them as new — the recursive-search version of this is why a deep search inflates its source count without growing the corpus. Distill to what the next stage needs to *decide* something, and carry the provenance separately.
- **Say what each agent knows in the skill's design.** Write it down in the SKILL.md alongside the pattern: what goes in each spawned agent's prompt, what is deliberately withheld, and why. A future maintainer who can't tell which omissions were deliberate will helpfully "fix" the isolation that was doing the work.
