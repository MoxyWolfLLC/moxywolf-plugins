...
4. **Run the docs-first protocol** with the mandatory docs-corpus defaults from graphify-core: `graphify extract <corpus> --backend openrouter --mode deep --token-budget 4000 --max-concurrency 16 --no-cluster` (provider-default model is fine; the small chunk budget is the load-bearing fix). There is no keyless fallback for docs. Mind the cache trap: after changing mode/model/budget, `rm -rf <corpus>/graphify-out` before re-running. **Whole-vault runs (hundreds of notes) must run host-side via Desktop Commander** — the sandbox time box kills the cache between calls. Then `cluster-only` + `label` per the core protocol.
