...
argument-hint: <path> [--keyless] [--with-docs] [--out <dir>]
---
...
1. Parse `$ARGUMENTS`: the target path (required), `--keyless` (skip the keyed naming pass), `--with-docs` (run optional Pass C), `--out <dir>` (where `graphify-out/` lands; default: alongside a scratch corpus, with the report surfaced in chat).
