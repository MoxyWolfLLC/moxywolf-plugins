...
3b. **graphify enrichment (run-first, Step 0).** Before the tree walk, generate the graph per the skill's "Structural enrichment — graphify knowledge graph (run-first)" protocol: install the `graphifyy` CLI if needed, then extract to `<clone>/graphify-out` — keyed path (`graphify extract <clone> --backend <auto>` + `graphify label`) when an LLM key is available, else the keyless code-only path (`graphify extract <code-scope> --no-cluster` + `graphify cluster-only <code-scope> --no-label`). Then parse `graph.json` and let its god nodes (core abstractions) and communities (module decomposition) drive the PRD's architecture and component sections, with its import cycles / isolated nodes feeding the health report's Technical Debt section. Skip only on `--skip-graphify`, `--quick`, or when a usable graph already exists (or a `--graphify-graph <path>` argument is supplied). If graphify can't be produced, fall back to the tree walk and note it — never block. Weight by repo type: strong signal for code-heavy repos, weak hint for markdown/config-heavy ones.
