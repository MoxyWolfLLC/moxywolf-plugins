...
- **graphify enrichment (run-first on deep analysis, delegated)** — As Step 0 of a `deep` analysis the plugin builds a full-repo corpus from the clone and runs the [graphify](https://github.com/safishamsi/graphify) knowledge graph via the standalone **graphify plugin's `graphify-core` skill** (the canonical runner protocol: team OpenRouter key per DR-010, sandbox-safe two-pass AST-then-label split, keyless code-only fallback), then grounds the architecture and technical-debt analysis on its god nodes, communities, import cycles, and isolated nodes. It degrades gracefully — any graphify failure falls back to the plain tree walk and is noted in the report. Skippable via `--skip-graphify` or `quick` mode; an existing graph or `--graphify-graph <path>` is reused as-is. Weighted by repo type (strong for code-heavy repos, a weak hint for markdown/config-heavy ones). See the MoxyWolf Tech Stack note `tool-graphify-knowledge-graph.md`.
...
- For deeper REST calls, install the GitHub CLI (`gh`) or set `GITHUB_TOKEN` in your shell. The `gh` path is preferred — it reuses your already-authenticated session and handles pagination cleanly.
