...
- **graphify enrichment (run-first on deep analysis)** — As Step 0 of a `deep` analysis the plugin generates a [graphify](https://github.com/safishamsi/graphify) knowledge graph at the clone's `graphify-out/graph.json`, then grounds the architecture and technical-debt analysis on its god nodes, communities, import cycles, and isolated nodes. It installs the `graphifyy` CLI if needed and runs the **keyless code-only path** by default (numbered communities, fully offline); when an LLM key is available it runs the **keyed path** for named communities and multimodal (docs/image) concept nodes. It degrades gracefully — any graphify failure falls back to the plain tree walk and is noted in the report. Skippable via `--skip-graphify` or `quick` mode; an existing graph or `--graphify-graph <path>` is reused as-is. Weighted by repo type (strong for code-heavy repos, a weak hint for markdown/config-heavy ones). This run-first behavior supersedes the original read-if-present scope of DR-005; see the MoxyWolf Tech Stack note `tool-graphify-knowledge-graph.md`.
