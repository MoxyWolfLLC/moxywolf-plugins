...
connector doesn't expose balance_due, subtract known Stripe settlements from the
invoice total before including it.

---

## 2. Stripe payout lag varies by account and transaction type

**Bad:** Assuming all Stripe charges pay out on a flat 2-day schedule. Stripe
holds funds differently for disputes, new accounts, and high-risk transactions,
and payout schedules vary (daily/weekly/monthly rolling) — a flat assumption
produces overconfident inflow timing.

**Good:** Compute payout lag from actual `created` (charge) → `arrival_date`
(payout) pairs in the Stripe balance-transaction history. Use the computed mean
and stddev per payout schedule.
...
**Bad:** Dividing stddev by a mean lag of 0 (e.g. immediate-settlement charges
like Stripe Tap to Pay) produces a divide-by-zero error or an infinite band.
