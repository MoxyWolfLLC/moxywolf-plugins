...
**Customer paid via check or bank transfer — not visible in Stripe.**

The Stripe cross-reference only catches Stripe payments. A customer who paid by check or ACH may still appear as overdue in AR. Note this in the summary: "Stripe history only — check/ACH payments not verified." Let the owner confirm before sending.
...
**Stripe reminder send fails for customers without a Stripe account.**

Stripe reminders only work if the customer has an active Stripe account. If Stripe returns a send error, fall back to queuing a mail draft and report the fallback: "Stripe send failed for [customer] — queued as [mail app] draft instead." Do not silently drop the reminder.
...
**Stripe API returns 429 rate limit errors.**

Stripe's MCP connector rate-limits aggressively when the requested date window is wide. The most common cause is querying 14–30 days of transactions in a single call.
...
*Fallback:* If the 3-day retry also returns 429, skip the Stripe cross-reference for this run entirely. Flag every customer in the batch as "Stripe unavailable — verify manually" in the summary table. Proceed with QuickBooks-only scoring. Do not silently drop the caveat — the owner needs to know the cross-reference was skipped before approving any sends.
