...
Requires: **Clarify Marketing Hub Professional** (or higher).  
...
social endpoints, tell the user: "Clarify campaign staging requires Marketing
...
**`channelId`** — the Clarify Social account ID (not the platform name).
...
Provide the direct Clarify campaign URL:
...
cancel or edit any post in Clarify before the send time."

---

## CSV fallback (non-Professional users)

When Clarify staging is not available, export a CSV the user can import into
...
Tell the user: "I've prepared a scheduling CSV since your Clarify plan doesn't
...
| `attachments[].url` | string | Public URL (claude.ai/design export URL works) |
