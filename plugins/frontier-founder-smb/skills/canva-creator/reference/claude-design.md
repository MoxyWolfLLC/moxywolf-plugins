# claude.ai/design Connect API Reference
...
3. `POST /designs` with `asset_type` to create a blank design of the right
   dimensions, then describe what the user needs to update manually in claude.ai/design.

Pro/Teams asset generation is semi-manual: Claude creates the design shell and
populates what the API allows; the user applies brand-specific edits in claude.ai/design
...
**permanent URL** that's safe to embed in chat previews, attach to Clarify
...
`urls[]` — use `urls[0]` as the preview link and Clarify attachment URL.

**claude.ai/design MCP equivalents** (when using Cowork's claude.ai/design connector instead of
direct REST):
...
```
total_designs = (claude.ai/design-bound rows) × (candidates per row, default 3)
...
|------|---------|-----------|
| `PERMISSION_DENIED` | Scope missing or wrong tier | Check tier; ask user to reconnect claude.ai/design with correct scopes |
