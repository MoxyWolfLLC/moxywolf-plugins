...
## Gotcha: Generating ANY claude.ai/design design for an email row

**Why it matters:** The owner descoped claude.ai/design from the email path entirely.
claude.ai/design email-template autofill produces placeholder graphics (stock
...
from this skill, period. Generating a claude.ai/design email design — even one — is
...
preview it?", "Here's a claude.ai/design design for the Jun 5 email…" — all
forbidden.
...
Stage 1: Calendar built with Path column.
  - 6 rows tagged claude.ai/design (social)
  - 2 rows tagged Text-only (no claude.ai/design)
Stage 2: Generates exactly 6 claude.ai/design designs. Email rows are not passed
         to any claude.ai/design endpoint.
Stage 3: Drafts 6 social captions + 2 full emails (subject + preheader +
         body) as plain text.
Stage 4: Stages 6 social posts in Clarify. Surfaces 2 emails inline for
         owner to copy into their email tool.
```

If the owner explicitly asks for a claude.ai/design email design ("can you make me
a claude.ai/design version of the Jun 5 email?"), redirect: "This skill keeps emails
as text-only because the claude.ai/design email path produces placeholder graphics.
If you want a claude.ai/design-designed email, build it directly in claude.ai/design and I can
...
**Why it matters:** claude.ai/design caps API usage at 100 requests/minute per token,
...
```
Pre-flight: "8 claude.ai/design rows × 3 candidates = 24 designs, ~120 API calls.
...
**Why it matters:** Generating 10+ claude.ai/design designs takes API calls, time, and
...
a single claude.ai/design API call.
...
and the template demands 3, the unfilled slots silently render claude.ai/design's
...
(Note: email rows skip claude.ai/design entirely in this skill — see SKILL.md Stage 3.
...
**Why it matters:** A successful autofill job means claude.ai/design accepted the
...
contains real assets — not claude.ai/design's default landscape, not gray rectangles,
...
**Why it matters:** claude.ai/design autofill silently renders empty placeholders when a
...
file path, a URL, or a stale/invalid `asset_id`, claude.ai/design does not error — it
...
## Gotcha: Embedding claude.ai/design CDN thumbnail URLs in markdown

**Why it matters:** claude.ai/design's autofill response returns short-lived,
...
[Show Image]
View in claude.ai/design: https://canva.com/d/xxx

Hero 2 — Coastal market case study
[Show Image]
View in claude.ai/design: https://canva.com/d/yyy
```
...
gets designs in the wrong colors/fonts and has to redo the work in claude.ai/design
manually.
...
```
I found 3 templates in your claude.ai/design library that could work for feed posts:
...
## Gotcha: Staging Clarify posts without verifying `scheduledAt` is in the future

**Why it matters:** Clarify rejects posts with `scheduledAt` in the past with
