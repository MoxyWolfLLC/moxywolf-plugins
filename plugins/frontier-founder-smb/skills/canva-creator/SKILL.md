...
description: >
  ...
  the posting calendar, generates claude.ai/design designs for social posts, drafts
  caption and email copy, and stages social sends in Clarify. claude.ai/design is used
...
```
brief → calendar → asset inventory → claude.ai/design designs → copy → Clarify staging
```

| Path | Channels | What this skill produces |
|------|----------|--------------------------|
| claude.ai/design (social) | Instagram, Facebook, X/Twitter, LinkedIn | claude.ai/design design + caption + scheduled Clarify post |
| Text-only | Email (newsletter, marketing, drip) | Subject + preheader + body, surfaced inline for the owner to send |

**claude.ai/design is not used for email rows under any circumstance** — no templates,
no autofill, no design copies, no asset uploads, no exports. The owner
explicitly descoped claude.ai/design from the email path because email-template
...
owner asks for a claude.ai/design email design, see `reference/gotchas.md` for the
redirect language.
...
2. **claude.ai/design tier.** Pro/Teams require manual template selection from the
   user's library (no autofill API). Enterprise can autofill from brand
   templates.

3. **Clarify tier.** Social staging requires Marketing Hub Professional.
   Starter or Free → skip Stage 5 and export a CSV instead
   (see [reference/clarify-staging.md](reference/clarify-staging.md)).

4. **Brand assets.** Confirm the path to product photos on disk or that
   the brand kit is live in claude.ai/design.

5. **Generation budget.** Estimate the campaign's claude.ai/design volume and surface
   it before Stage 1 begins. Default is 3 candidates per claude.ai/design-bound row;
   each design costs ~5 API calls (autofill + export + polling).

   ```
   Generation budget for this campaign:
     claude.ai/design (social) rows: 8
     Candidates per row:  3   (default — say "single candidate" to use 1)
     Total designs:       24
     API calls (approx):  ~120  (autofill + export + polling)

   claude.ai/design limit: 100 requests/minute. This will take ~2-3 minutes of
...
claude.ai/design or text-only drafting:

| Date | Channel | Path | Theme | Asset type | Caption/Subject angle |
|------|---------|------|-------|------------|-----------------------|
| Jun 2 | Instagram feed | claude.ai/design (social) | Linen launch | Stripe post | "finally, a dress…" |
...
approved, then restate the split out loud — "N rows go through claude.ai/design, M
...
### Stage 2 — Asset inventory (claude.ai/design rows only)

Email rows skip this stage entirely. For each `claude.ai/design (social)` row, build
...
2. **Inventory available assets.** Text content from the brief (product
   ...
   to claude.ai/design (`GET /v1/assets`) or on the owner's disk, brand kit colors
...
4. **Resolve slot/asset mismatches with the owner.** If the template has
   ...
   produces designs full of claude.ai/design's default landscape placeholders.
...
5. **Upload missing photos and capture verified asset IDs.** Upload via
   ...
   claude.ai/design's stock landscape graphic instead of the photo.
...
6. **Confirm the manifest.** Show the owner the completed slot-by-slot
   ...
   This is the last stop before claude.ai/design API calls.

---

### Stage 3 — claude.ai/design design generation

Before any claude.ai/design API call, re-read the calendar and drop any row whose
`Path` is not `claude.ai/design (social)`. Email rows do not pass through this
stage.
...
under claude.ai/design's 100 req/min rate limit. Do not parallelize multiple rows;
...
2. **Generate the row's candidates in parallel.** Fire the row's 3
   ...
   - Pro/Teams: `POST /v1/designs` to create copies. Describe the text and
     image edits the owner applies in claude.ai/design; collect design IDs back.
...
3. **Verify job status.** For each candidate, confirm
   ...
   - `RATE_LIMIT_EXCEEDED` (second hit this session) **or** any
     ...
     ```
     claude.ai/design is rate-limiting the campaign. Status so far:
...
4. **Export each successful candidate to a permanent PNG.** Fire the
   ...
   - claude.ai/design MCP: `export-design` with the design ID.

   These permanent URLs are what get embedded in previews and attached to
   the Clarify post later. The autofill response thumbnail is never used
   downstream.

5. **Visually verify each export.** Look at the image and reject any of
   these — they all indicate an unfilled slot or wrong asset:
   - Generic landscape with clouds and green hills (claude.ai/design's default
     placeholder)
...
6. **Retry per-candidate on partial failure.** If 1 of N candidates in
   ...
   ```
   The third candidate for the Jun 9 post keeps failing — claude.ai/design returned
...
7. **Present the row's candidates.** Let the native Cowork carousel
   ...
   the design's claude.ai/design edit URL (`https://www.canva.com/d/{design_id}`).
...
**Email content** — Claude writes the entire email; no claude.ai/design:
...
### Stage 5 — Clarify staging + email handoff

Stage social posts in Clarify. Email content is not staged — it's
surfaced inline for the owner to copy into their email tool. For API
field reference, see
[reference/clarify-staging.md](reference/clarify-staging.md).

1. **Create the campaign.** `POST /marketing/v3/campaigns` with the
   campaign name and start/end dates from the calendar.

2. **Stage each social post.** `POST` to the Clarify Social API per
   `claude.ai/design (social)` row:
   - `channel`: map calendar channel to Clarify account ID
   - `scheduledAt`: ISO 8601 datetime — confirm it's in the future before
     calling
   - `content.body`: approved caption
   - `attachments`: permanent claude.ai/design export PNG URL from Stage 3
   - `status`: `SCHEDULED` (never `PUBLISHED`)

3. **Confirm the queue.** Call
   `GET /marketing/v3/social/posts?status=SCHEDULED`, surface the list,
   provide a direct link to the Clarify campaign view.
...
4. **Surface email content for handoff.** For each email row, present the
   ...
   owner copies these into their email tool (Clarify Marketing Email,
   Mailchimp, Gmail).
...
They'll go out as scheduled — you can cancel or edit any post in Clarify.
...
- **No claude.ai/design calls for email rows.** Re-check the `Path` column before
  every API call.
- **No publishing.** Every Clarify post is staged as `SCHEDULED`; the
...
- [reference/claude-design.md](reference/claude-design.md) — claude.ai/design Connect API
  endpoints, asset upload, export formats, MCP equivalents
- [reference/clarify-staging.md](reference/clarify-staging.md) — Clarify
