...
## Gotcha: Creating a duplicate contact when the email is already in Clarify under a different case

**Why it matters:** Clarify dedupes on exact email match. `Sarah.Lin@acme.com` and `sarah.lin@acme.com` are treated as distinct, but an owner who already created one will see a silent duplicate appear. That destroys the owner's trust in the skill.

### ✗ Bad

> Searched Clarify for `Sarah.Lin@acme.com` — no match. Creating new contact: Sarah Lin (Sarah.Lin@acme.com).

New contact is created; owner's existing `sarah.lin@acme.com` contact now has a duplicate.

### ✓ Good

> Searched Clarify for `sarah.lin@acme.com` (case-insensitive) — no match. Creating new contact: Sarah Lin (sarah.lin@acme.com). Announcing before write.
...
**Why it matters:** Clarify activities are signal surfaces for fast scanning, not a transcript archive. A 12-message thread pasted into the activity body is useless; a three-sentence summary of what was agreed is actionable.
...
**Why it matters:** Duplicate deals are very hard to untangle in Clarify — they split activity history, confuse reporting, and the owner usually notices weeks later when a forecast looks wrong.
