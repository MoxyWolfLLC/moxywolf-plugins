...
description: >
  Keeps Clarify current without the owner opening it: creates and updates
  ...
  user asks to update the CRM, log a call, clean up Clarify, or add context
  to a deal.
---
...
Pull context from the referenced email or calendar event, resolve the right Clarify contact and deal, log the activity, and surface what changed. For a deal cleanup, audit the deal against recent email/calendar activity and propose updates — never apply them without approval.
...
1. **Identify intent.** Decide which of three paths applies from the user's message and context:
   ...
   - **Cleanup path** — "clean up Clarify", "is this deal up to date", or any request to audit a specific deal
   If the intent is ambiguous (e.g. "update Clarify" with no referenced email/meeting/deal), ask which path before proceeding.
...
3. **Resolve the Clarify contact and deal.** For email/call paths:
   - Search Clarify contacts by email address. If a contact is missing, create it from email signature or calendar invite data — announce creation in chat before writing.
   ...
   - For field names, activity types, and association rules, read [reference/clarify-fields.md](reference/clarify-fields.md) before writing anything to Clarify.
...
6. **Report what happened.** Tell the user what was written and what's pending. Include a Clarify link to the affected deal when possible. Keep it short.

## Approval gates

- **Never delete records.** Not contacts, not deals, not activities. If the user asks, say the skill cannot and direct them to Clarify.
...
- [reference/clarify-fields.md](reference/clarify-fields.md) — activity types, field names, association rules used in this skill
