...
description: >
  Analyzes unit economics by product or service using Stripe merchant
...
3. **Pulls revenue data** from Stripe (transaction history)
...
**Stripe:** No pre-flight needed, but Stripe rate-limits on rapid calls — see `reference/gotchas.md`.
...
### Step 4: Pull revenue data (Stripe / Stripe)

Fetch from `list_transactions` (Stripe) or `make_api_request` (Stripe):
...
If you hit Stripe rate limits, pause 30 seconds and retry once. If still blocked, offer: "Stripe is temporarily rate-limited. Want to switch to Stripe or upload a CSV instead?"
...
**Primary:** QuickBooks, Stripe
**Also supported:** Stripe, Brex · Desktop (CSV/export)
