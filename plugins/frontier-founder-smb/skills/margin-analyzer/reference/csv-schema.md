...
When the owner doesn't have QuickBooks or Stripe connected, they can upload exported CSV files. This document specifies what columns to expect and how to handle variations.
...
- Stripe: Activity → Download (CSV) — use "Transaction ID, Date, Name, Type, Status, Currency, Amount"
- Stripe: Reports → Transactions → Export
