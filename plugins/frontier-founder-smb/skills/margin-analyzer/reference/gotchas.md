...
An owner sees $50K in Stripe transactions and thinks that's what they made. It isn't — it's what they collected.
...
### ✗ Bad
"Your Stripe revenue last quarter was $50,000."

### ✓ Good
"Your Stripe revenue last quarter was $50,000. After direct costs of $31,000, your gross profit was $19,000 — a 38% gross margin."

---

## Gotcha: Using list price instead of effective price

Owners often have discounts, refunds, and promotions that reduce what they actually receive per transaction. Stripe transaction data reflects actual collected amounts, but QuickBooks invoices may show list price.

**Why it matters:** If you compute margins using list price but Stripe shows actual collections, your cost-per-unit math will look better than reality.

### ✗ Bad
Use invoice amounts from QB for revenue and Stripe costs for COGS → margin appears inflated.

### ✓ Good
Use Stripe transaction amounts as the revenue source (actual collected), and QB for costs. Note any discrepancy between QB invoice totals and payment totals — it's worth surfacing.
...
## Gotcha: Stripe rate-limiting on repeated calls

Rapid repeated calls to `list_transactions` (e.g., iterating through multiple date ranges) can trigger Stripe's rate limiter.
...
### ✓ Good
Call `list_transactions` → if 429, pause 30 seconds → retry once → if the retry succeeds, continue normally, but treat any *second* 429 in the same session as a signal to stop retrying. After a second rate-limit event (even if separated by a successful call), immediately surface the fallback: "Stripe is rate-limiting repeated calls in this session. I can switch to Stripe for the revenue data, or you can upload a Stripe CSV export — either works. What would you prefer?" Do not attempt a third retry.
