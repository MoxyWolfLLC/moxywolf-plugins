...
description: >
  Analyzes sales data from Stripe and QuickBooks to find top performers and
...
1. **Pulls sales data** from QuickBooks or Stripe (transaction history, product/service revenue by date)
...
**Note:** Stripe do not require profile setup.
...
Fetch data from the authenticated connector (QuickBooks, Stripe, user's choice):
...
- **Stripe:** Fetch merchant transactions via `list_transactions`. *Rate-limiting:* If you hit rate limits, pause 30 seconds and retry once. If still blocked, gracefully offer: "Stripe is rate-limited. Would you like to switch to QuickBooks instead, or I can continue with historical data I already pulled?"
- **Stripe:** Requires location ID first. Call `make_api_request(service="locations", method="list")` to discover available locations, then fetch orders for each location. *Future enhancement:* Stripe integration is stubbed; full path documented in `reference/stripe-integration.md`.
