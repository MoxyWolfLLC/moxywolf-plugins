...
If you call `list_transactions` multiple times in rapid succession (e.g., looping through date ranges), Stripe may rate-limit and return 429 errors.
...
### ✓ Good
Call list_transactions → 429 error → Wait 30 seconds → Retry → If still blocked, offer fallback: "Stripe is temporarily rate-limited. Would you like to switch to QuickBooks, or continue with partial data?"
