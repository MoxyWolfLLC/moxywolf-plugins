...
**Stripe transactions already in QB** — if the user has the Stripe-QB auto-sync
enabled, Stripe transactions will already appear in the register. Don't double-count
them during reconciliation (Step 3). Check whether the QB deposit lines have
"Stripe" in the account or memo before matching.
