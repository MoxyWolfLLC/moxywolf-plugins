...
The Stripe connector returns a **Transaction History** or **Settlement** report.
Key fields:

| Field | Notes |
|---|---|
| `transaction_info.transaction_id` | Stripe transaction ID |
...
| `transaction_info.fee_amount.value` | Stripe fee (negative) |
...
**Deposit date vs. transaction date:** Stripe batches payouts. A sale on April 28
...
aggregate flags. A QB deposit that doesn't match Stripe may legitimately be a
