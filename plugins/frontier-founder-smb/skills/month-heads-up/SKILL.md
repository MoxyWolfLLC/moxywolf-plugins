...
2. Pull Stripe settled balance and pending payouts.
...
If QuickBooks is unreachable, stop — the cash forecast requires QB as the source of truth. If Stripe is missing, run the forecast from QB-only data and note "Stripe not connected — Stripe receivables excluded from forecast." Same for Stripe if missing.

## Approval gates

- **Never initiate payments or send emails automatically.** Surface the data and actions for the owner to take.
- **Never project revenue that hasn't been confirmed in QB or Stripe.** Use conservative estimates only.
