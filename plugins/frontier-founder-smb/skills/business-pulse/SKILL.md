...
description: >
  ...
  cash position (QuickBooks), sales trend (Stripe), pipeline movement
  (Clarify), this week's commitments (Calendar), urgent watch-list items
...
- **Stripe / Stripe** — 7-day settlements, sales trend, failed/pending transactions
- **Clarify** — pipeline by stage, deals moved/closed, deals gone cold, new leads
...
- **Shopify / Stripe** — fulfillment issues (if connected)
...
- **Pipeline coverage** — Clarify weighted pipeline ÷ monthly revenue target
- **Revenue trend** — this month's QBO revenue vs. prior month (or 7-day Stripe vs. prior 7 days)
...
- Clarify deals with no activity in 7+ days, or close date in past but still open
- Gmail threads marked urgent or containing "escalation," "complaint," "cancel," "refund"
- Failed or pending Stripe transactions > $500
...
Cross-connector synthesis is where this skill earns its keep. If a Slack message connects to a stalled Clarify deal, surface that link in the #1 Priority section. Synthesis is what makes the pulse more useful than checking each tool separately.
