...
## Revenue & Sales (Stripe / Stripe / Stripe)

| Metric | Tool | Notes |
|---|---|---|
| 7-day settlement total | Stripe transactions | Sum completed settlements in window |
| Sales trend | Stripe transactions | This 7 days vs. prior 7 days; compute delta |
| Failed / pending transactions | Stripe transactions | Flag any > $200 |
| Stripe settlements | Stripe (if connected) | Same as Stripe — sum + trend |
| Stripe revenue | Stripe `list_invoices` (if connected) | Paid invoices in window |

Use whichever payment connectors are available. If both Stripe are connected, report combined and per-source.
...
| Stalled deals | Clarify | Open deal, no activity 7+ days |
| Slipped deals | Clarify | Open deal, closedate in past |
| Urgent Gmail threads | Gmail | `is:important` or escalation keywords |
| Failed payments | Stripe / Stripe | Failed or pending > $200 |
