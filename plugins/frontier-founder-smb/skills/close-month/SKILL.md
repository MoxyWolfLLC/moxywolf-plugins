...
2. Pull settlements from each connected payment processor (Stripe) for the same month.
3. Match QB entries to processor settlements by amount + date (±2 days).
4. Surface three gap categories:
   - **Unmatched processor settlements** — money came in via Stripe but never landed in QB
...
If QuickBooks is unreachable, stop — reconciliation requires QB as the source of truth. If a payment processor (Stripe) is unreachable, run reconciliation against the available processors and note "Stripe not connected — Stripe settlements skipped from reconciliation" (or whichever is missing). If all processors are missing, run QB-only analysis and flag it.
