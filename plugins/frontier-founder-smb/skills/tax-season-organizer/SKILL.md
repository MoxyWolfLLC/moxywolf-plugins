...
description: >
  ...
  year-end 1099 prep — scans QuickBooks, Stripe for contractors paid
...
→ Pull all contractor/vendor payments from QuickBooks + Stripe + Stripe
...
**Stripe:** Pull all "Goods & Services" payments sent. Note: Stripe issues its own 1099-K to contractors above the threshold — flag these separately in output so the accountant can determine whether a 1099-NEC is also needed.

**Stripe:** Pull all transfers/payouts made to external parties. Same 1099-K caveat as Stripe applies.
...
3. **1099-NEC candidates table** — Columns: payee name, total paid, data
   ...
   payee paid via Stripe with a note that the platform may issue
   its own 1099-K.
...
6. **Payment processor note** — Explain that Stripe issue their own
...
- [reference/connector-queries.md](reference/connector-queries.md) — how to pull data from QuickBooks, Stripe
