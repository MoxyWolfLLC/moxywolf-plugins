...
**Stripe** (goods & services payments sent, 2024):
...
Cross-referencing QuickBooks and Stripe:

| Payee | QuickBooks | Stripe | Total | Notes |
|-------|-----------|--------|-------|-------|
...
- Stripe overlap: 2 contractors (Jenna Torres, Design by Mike) — see note below

### 1099-NEC candidates

| Payee | Total paid | Source(s) | W-9 status | Notes |
|-------|-----------|-----------|------------|-------|
| Jenna Torres | $9,600 | QB + Stripe | ✅ EIN on file | Stripe also paid $1,200 — confirm 1099-K handling |
...
| Design by Mike | $2,100 | Stripe only | ⚠️ Missing W-9 | Full name unknown — obtain before filing |
| Bob Nguyen | $1,030 | QB + Stripe | ⚠️ Missing W-9 | Combined QB ($550) + Stripe ($480) = $1,030 |

### Missing W-9 action list
These contractors must provide a W-9 before you can file:

1. **Design by Mike** — paid $2,100 via Stripe. Obtain full legal name and W-9.
   IRS W-9 form: https://www.irs.gov/pub/irs-pdf/fw9.pdf
2. **Bob Nguyen** — paid $1,030 (QB + Stripe). W-9 not on file for either source.

### Payment processor note (Stripe)
Jenna Torres and Design by Mike received payments via Stripe. Stripe issues its own
...
Stripe's 1099-K covers the reporting obligation.
...
- [ ] Confirm 1099-K overlap handling for Jenna Torres and Design by Mike (Stripe)
