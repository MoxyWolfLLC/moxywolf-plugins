...
3. Pull Stripe settlements for the same period to validate QB revenue.
4. Calculate: revenue growth %, margin change in points, top 3 revenue categories.

## Step 2 — Customer health

1. Pull Clarify deal data: new customers won, churned, average deal size, pipeline entering next quarter.
...
If QuickBooks is unreachable, stop — the QBR requires QB financial data as the foundation. If Stripe is missing, skip cross-validation and note "Stripe not connected — revenue validated from QB only." If Clarify is missing, skip customer health (Step 2) and note "Clarify not connected — customer health section skipped."
