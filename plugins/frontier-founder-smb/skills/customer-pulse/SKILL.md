...
version: 0.2.0
description: >
  Aggregates Stripe disputes, Clarify feedback and tickets, and email
...
2. **Pull Stripe disputes.** Fetch disputes opened in the window. If the Stripe API returns a rate-limit error, skip and add `Stripe: rate-limited — not included` to the Sources section. Do not retry; do not error. See [reference/gotchas.md](reference/gotchas.md) for the rate-limit pattern.

3. **Pull Clarify tickets and feedback.** Fetch open and recently closed tickets. If 0 tickets exist, record `Clarify tickets: 0` and continue — do not surface a warning.
...
7. **Extract themes.** Group all evidence into 3–5 recurring themes. Each theme must include:
   ...
   - 2–3 verbatim quotes with source tags: `[Stripe]`, `[Clarify]`, `[Gmail]`, `[Intercom]`, or `[Review]`
...
9. **Deliver the report.** Structure the output with these sections in order:
   ...
   - **Sources pulled** — Bullet list with signal counts per source (Stripe
     disputes, Clarify tickets, Gmail threads, Intercom conversations, pasted
...
- [reference/gotchas.md](reference/gotchas.md) — Stripe rate limits, Clarify empty state, verbatim quote requirement, Gmail keyword drift
