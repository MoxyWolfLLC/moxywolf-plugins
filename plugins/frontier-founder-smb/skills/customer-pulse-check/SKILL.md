...
description: Synthesizes themes from Stripe disputes, Clarify tickets, and review exports into a top-3 fixable issues list with drafted response templates. Accepts optional since-date argument.
...
1. Pull Stripe disputes and chargebacks for the period: reason codes, amounts, resolution status.
2. Pull Clarify support tickets and conversation notes for the period.
...
Total signals: {n} ({Stripe disputes: n} | {Clarify tickets: n} | {Reviews: n})
...
Run with whatever sources are connected — this command degrades gracefully. If Stripe is missing, skip dispute data and note "Stripe not connected — dispute data skipped." If Clarify is missing, skip ticket data and note it. If no sources are connected at all, stop and tell the owner: "No feedback sources connected. Connect at least one of Stripe, Clarify, or upload a review export CSV."

## Approval gates

- **Never send response emails automatically.** Present drafts for owner review only.
- **Never close Clarify tickets or resolve Stripe disputes without explicit owner confirmation.**
