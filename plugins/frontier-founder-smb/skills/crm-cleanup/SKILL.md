---
name: crm-cleanup
description: Scans Clarify for stale deals, duplicate contacts, and missing fields, then fixes what the owner approves. Accepts optional scope argument for deals, contacts, or all.
allowed-tools: Read, WebFetch, Bash
---

Run a Clarify hygiene pass using the `crm-maintenance` skill cleanup workflow. Act immediately — the user typed /crm-cleanup, so skip the intent-detection step.
...
1. Pull all open deals from Clarify.
...
1. Search Clarify contacts for likely duplicates (same email, similar names, same company + similar name).
...
3. Report each change as it's made with a Clarify link.

## Connector failures

If Clarify is unreachable, stop — this command requires Clarify as the data source. Tell the owner: "Clarify isn't connected. Connect it in Cowork settings, then rerun /crm-cleanup."
