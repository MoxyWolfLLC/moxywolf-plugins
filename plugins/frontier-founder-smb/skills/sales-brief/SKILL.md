...
1. Pull Stripe transactions for the lookback period grouped by item/service/SKU.
...
- Cross-reference with Clarify campaign activity for the period
...
If both QuickBooks and Stripe are unreachable, stop — sales analysis requires at least one revenue source. If only one is connected, run from that source and note "QuickBooks not connected — revenue data from Stripe only" (or vice versa). If Clarify is missing, skip campaign cross-reference in the "why analysis" and note it.

## Approval gates

- **Never auto-schedule or publish content.** The brief is for owner review only.
- **Never create claude.ai/design assets automatically** — offer to generate them after owner approves the brief.

## Output

Present the sales analysis, then the content brief. Ask the owner if they'd like to generate claude.ai/design assets for any of the planned posts.
