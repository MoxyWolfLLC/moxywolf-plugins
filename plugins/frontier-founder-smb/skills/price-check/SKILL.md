...
3. Pull Stripe gross sales for the same products to cross-validate.
...
If QuickBooks is unreachable, stop — margin analysis requires QB revenue and cost data. If Stripe is missing, run from QB-only and note "Stripe not connected — cross-validation against Stripe sales skipped."
...
- **Never update any prices in QB, Stripe, or any connected system.**
