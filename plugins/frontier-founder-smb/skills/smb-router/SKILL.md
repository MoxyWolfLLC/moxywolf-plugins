...
name: smb-router
description: >
  The front door to the Frontier Founder SMB (FFSMB) plugin. Listens to what the
  owner needs right now — vague or specific — and routes them to the best skill or
  slash command for the moment. Also serves as a guide: explains what's available,
...
| "Clean up the CRM" / "Clarify is a mess" / "stale deals" | `/crm-cleanup` |
| "Review this contract" / "NDA" / "should I sign this?" | `/review-contract` |
| "I'm hiring" / "write a job post" / "make an offer letter" | `/job-post-builder` |
...
**Finding new customers (companion plugin):**
| Owner says something like... | Route to |
|---|---|
| "find new leads" / "prospect" / "build a cold outreach list" / "fill my pipeline" | Clarify's native `find-leads` for a quick search; for deeper enrichment + multi-step sequences, recommend the **Apollo** plugin (`apollo:prospect`) if installed |
...
**Good:**
> "Sounds like you want to see where your money is going before month-end. I'll run `/close-month` — it reconciles QuickBooks against Stripe and flags anything that looks off. Want me to start?"
...
Group into buckets and lead with the one most relevant to their stored headaches:
...
**Your contracts & hiring:** `/review-contract` · `/job-post-builder`
...
2. If a fallback command can serve the same intent with the connectors that *are* connected, offer it: "Without QuickBooks, I can still run `/friday-brief` using your Stripe data — it won't be as complete, but you'll get a revenue snapshot."
3. Always be explicit about what's skipped: "Note: Stripe isn't connected, so the revenue cross-validation will be skipped."
...
|---|---|---|
| `/plan-payroll` | QuickBooks | Stripe |
| `/close-month` | QuickBooks | Stripe |
| `/month-heads-up` | QuickBooks | Stripe |
| `/price-check` | QuickBooks | Stripe |
| `/tax-prep` | QuickBooks | Stripe |
| `/call-list` | Clarify | Gmail, Google Calendar, Apollo (enrichment) |
| `/run-campaign` | Clarify | QuickBooks, Stripe, claude.ai/design |
| `/sales-brief` | QuickBooks or Stripe | Clarify |
| `/crm-cleanup` | Clarify | Apollo (enrichment) |
| `/customer-pulse-check` | Stripe or Clarify | — |
| `/review-contract` | — (works with file upload) | DocuSign |
| `/job-post-builder` | — (works standalone) | DocuSign |
| `/monday-brief` | — (degrades gracefully) | QuickBooks, Stripe, Clarify, Calendar, Gmail |
| `/friday-brief` | Stripe or Clarify | — |
| `/quarterly-review` | QuickBooks | Stripe, Clarify |
| `/handle-complaint` | — (works with pasted text) | Gmail, Clarify, Stripe |
| `smb-onboard` | — | all |
...
1. If it's net-new B2B lead acquisition, point to Clarify's `find-leads` or the Apollo companion plugin.
2. If it's genuinely outside scope, say so plainly: "That's outside what I can help with right now. Here's what I'm good at:" and give the bucket overview from Step 4.
