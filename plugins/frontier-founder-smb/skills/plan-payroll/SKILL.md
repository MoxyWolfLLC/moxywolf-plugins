---
name: plan-payroll
description: Forecasts cash, ranks overdue invoices, and stages Stripe reminders so the owner can confidently run payroll. Accepts optional horizon and payroll-date arguments.
...
1. Pull AR, AP, and historical cash timing from QuickBooks, Stripe (whichever are connected). Fall back to CSV upload if no connector is live.
...
1. Pull overdue invoices from QuickBooks and Stripe.
...
4. Stripe-issued invoices queue as Stripe-send drafts; non-Stripe invoices queue as Mail drafts.
...
- If a connector is unreachable (QuickBooks, Stripe, Mail), stop, report which connector failed, and ask whether to retry, fall back to CSV, or abort.
