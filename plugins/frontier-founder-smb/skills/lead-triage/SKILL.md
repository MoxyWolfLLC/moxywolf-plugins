...
version: 0.1.1
description: >
  Scores inbound Clarify leads by engagement signals, company fit, and urgency
...
Pull inbound leads from Clarify, score them, and surface a ranked call list with talking points. Drafts follow-ups and proposes calendar slots — never sends or books without owner approval.
...
1. **Pull leads from Clarify.** Fetch contacts with `lifecyclestage` = `Lead` or `MQL` and `hs_lead_status` ≠ `Unqualified`. Use the field list in [reference/clarify-scoring.md](reference/clarify-scoring.md). If Clarify is unavailable, stop: *"Clarify is disconnected — connect it and try again."*
...
3. **Score each lead.** Apply the four-dimension model in [reference/clarify-scoring.md](reference/clarify-scoring.md):
   - **Engagement** — email replies, opens, site visits in Clarify (last 30 days only)
...
- [reference/clarify-scoring.md](reference/clarify-scoring.md) — Clarify field names, scoring weights, ICP defaults
