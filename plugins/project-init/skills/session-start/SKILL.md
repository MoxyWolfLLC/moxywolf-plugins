...
g. **Behavioural hook liveness** — Some plugins inject a ruleset at session start rather than waiting to be invoked: gstack-execution's verification discipline, and ponytail's restraint layer. They are the rules most likely to fail *silently*, because a hook that does not fire looks exactly like one that fired and had nothing to say.
   ...
   - **active** — a `VERIFICATION DISCIPLINE ACTIVE — gstack-execution` block is in this session's context. Status line: `Verification discipline: ✓ active`. Note *where* it arrived: at session start, or attached to the first user prompt. In cloud (Cowork) sessions the second is normal — the runtime fires SessionStart before the synced plugins have registered their hooks (measured 2026-09-02: SessionStart at +0.0s, hooks registered at +7.8s), so gstack-execution ≥ 0.9.0 and ponytail ≥ 0.2.0 deliver their block on the first prompt instead. Either placement is ✓; say which.
   - **not loaded** — no such block is present anywhere in context, including the first prompt. Status line: `Verification discipline: ⚠ NOT LOADED — the checks are not in context`, followed by the remedy below.
   ...
   1. Version below 0.9.0 — this build cannot reach a cloud session (0.8.0 had only the SessionStart hook, which fires before synced plugins register; below 0.8.0 there is no hook at all). Refresh the marketplace (Cowork's **Refresh**, or `claude plugin marketplace update moxywolf-plugins`) so the synced snapshot picks up 0.9.0, then start a fresh session — the block should arrive with the first prompt.
   2. Version 0.9.0 or above and still nothing — on-computer, a `hooks` key added to an already-installed plugin may need a reinstall to register: `claude plugin uninstall gstack-execution` then `claude plugin install gstack-execution@moxywolf-plugins`. In the cloud, read the runtime's own log first: `/tmp/claude-preload.out` carries `Registered N hooks from M plugins` and any `hook-load-failed` lines, and says whether the hook was registered at all.
   3. Still nothing after that and a fresh session — the wiring is wrong, not the install. Say so plainly rather than repeating the remedy.

   Whatever the state, the checks themselves are always readable: `gstack-execution/skills/gstack-execution/references/verification-checks.md` inside the plugin's install directory (the synced snapshot lives under `~/.claude/plugins/synced/<id>/`). When the line reads NOT LOADED, read that file in the same turn so the checks are in force for this session regardless.
...
- **Verification discipline reports `NOT LOADED`.** Surface it and carry on — it is a heads-up, not a gate. But do not quietly drop the line on later sessions if it stays absent: a warning that decays into silence is how the original problem returns. The checks themselves live in `gstack-execution`'s `skills/gstack-execution/references/verification-checks.md`; if the hook cannot be made to fire, read that file directly rather than working without them.
