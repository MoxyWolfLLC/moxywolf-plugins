...
6. **No mistakes found: skip the ledger.** If step 1 examined its sources and found nothing, do not create or touch the ledger. Write the handoff line with the coverage counts (see the template) and carry on. The validator refuses an empty append with exit 3, which is neither a pass nor a block.
