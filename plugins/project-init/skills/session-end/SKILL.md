...
- **Mounting issues — project's Taskade subfolder isn't currently mounted.** On-computer, use `mcp__cowork__request_cowork_directory` with the explicit path to mount it, then write. In the cloud (that tool is absent), you can't mount it yourself — ask the user to click **Add folder** in the Claude desktop app and select the subfolder's path, wait for the system-reminder confirming it's connected, then write. Detect what's mounted with `mcp__remote-devices__get_device_info` → `connectedFolders`.
