...
- **Mounting issues — project's Taskade subfolder isn't currently available.** Works online or offline: detect availability via `mcp__remote-devices__get_device_info` → `connectedFolders` when present, else probe the path by listing it. To get it mounted: if `mcp__cowork__request_cowork_directory` is present, call it with the explicit path; otherwise ask the user to add the path (**online:** the desktop app's **Add folder** button; **offline:** Cowork → Folders → Add), wait for the system-reminder / re-probe, then write. Don't hard-depend on `request_cowork_directory`.
