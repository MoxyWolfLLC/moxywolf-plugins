...
- **0.28.0** — The hook-liveness probe learns the **cloud case**. Its first live run (2026-09-02, OpenControls) reported `NOT LOADED` exactly as designed, and the runtime's own log showed why: SessionStart fired at +0.0s, the synced plugins' hooks registered at +7.8s. Neither remedy in 0.27.0's ladder could have fixed that. gstack-execution 0.9.0 and ponytail 0.2.0 now deliver on the first user prompt in that case, so Step 4g accepts the block wherever it arrived and reports which; the remedy ladder now runs refresh-to-≥0.9.0 → reinstall on-computer / read `/tmp/claude-preload.out` in the cloud → the wiring is wrong; the canonical checks path is corrected (`skills/gstack-execution/references/verification-checks.md`, not `references/…`); and on NOT LOADED the skill reads that file in the same turn so the checks are in force regardless.
- **0.27.0** — `/session-start` gains a **behavioural hook liveness** probe (Step 4g). gstack-execution's verification discipline and ponytail's restraint layer inject their rulesets at session start, and a hook that silently fails to fire looks exactly like one that fired with nothing to say — there was no detector for that. The briefing's Shared services line now reports `Verification discipline: ✓ active` or `⚠ NOT LOADED`, with the installed version via `ListPlugins` and an ordered remedy (refresh the marketplace → reinstall the plugin → the wiring is wrong, say so). The probe reads what is in this session's context, not the disk, and is explicitly forbidden from inferring a ✓ from the plugin merely being installed. Never gates the session. The checks themselves are NOT duplicated here — they live in gstack-execution's `references/verification-checks.md`, and this only reports whether they arrived.
