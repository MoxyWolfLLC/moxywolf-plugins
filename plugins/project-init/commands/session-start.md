...
3. Ensures the three standard MoxyWolf roots — MoxyWolf Vault, GitHub, Taskade — are mounted for this session. It detects what's mounted via `mcp__remote-devices__get_device_info` (cloud) or by probing the paths directly (on-computer). For any root that's missing, it auto-mounts first: in the cloud it proactively requests all missing roots in one `mcp__remote-devices__device_request_folder_access` call (a single approval dialog); on-computer it mounts via `mcp__cowork__request_cowork_directory` (the user approves each). Only if that tool is unavailable, or the user declines, does it fall back to asking the user to **Add folder** manually.
...
5. Reads the project's own `04 – Backlog & Sprints/` folder and the team Jira board (project MOXY), strictly filtering the board to the exact Jira label(s) declared in the project's instructions (`Jira label(s):`, used as written, never derived; no label declared means no board query) — so only this project's tasks surface, never another project's. (Jira is the single board; the vault `KANBAN_VIEW.md` was retired 2026-07-16.)
