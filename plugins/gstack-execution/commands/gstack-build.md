...
Run `/gstack-peer-review --builder <tool>` with the packet built from the design doc: `outcome` = the item, `acceptance_criteria` = the item's criteria verbatim, `exclusions` = the doc's Constraints and settled decisions, `tests` = Step 3's commands and results, plus `ci_runs` = `[{repo, run_id}]` for the CI run at the reviewed head (the dispatcher fetches it; a run at another head is not evidence). The reviewer's surface then carries `dependencies/`, the files the change or a criterion names, and `evidence/`, the CI runs the dispatcher read itself, and each fix round passes `--ci-run <repo>=<run_id>` for the run at its new head, `release_owner` = the accountable human's GitHub login, one `{path, base=main, head=branch HEAD}` pair per repo. Follow that command's loop: substantiate, fix in scope, commit, **push and pull back (Step 4) after every fix commit**, disposition, next round.
