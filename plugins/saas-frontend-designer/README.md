...
The `baseline-ui` skill's design-fluency material — the anti-pattern bans, OKLCH color strategy, typography craft, the two-altitude AI-slop test, and the `reference/` craft library — is adapted and redistributed from [impeccable](https://github.com/pbakaus/impeccable) (Apache-2.0, Copyright 2025 Paul Bakaus). The page modes and five playbooks (`critique`, `clarify`, `onboard`, `animate`, `craft-floor`) were added from impeccable v4.3.1 at `e0881d2` on 2026-09-22, modified to run without impeccable's engine. impeccable's engine, detector rules, live-browser runtime, scripts, hooks and standalone shell are not included. See `NOTICE` and `licenses/impeccable-Apache-2.0.txt`.
