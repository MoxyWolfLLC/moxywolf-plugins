...
The `baseline-ui` skill's design-fluency material — the anti-pattern bans, OKLCH color strategy, typography craft, the two-altitude AI-slop test, and the `reference/` craft library — is adapted and redistributed from [impeccable](https://github.com/pbakaus/impeccable) (Apache-2.0, Copyright 2025 Paul Bakaus). The page modes and five playbooks (`critique`, `clarify`, `onboard`, `animate`, `craft-floor`) were added from impeccable v4.3.1 at `e0881d2` on 2026-09-22, modified to run without impeccable's engine. Not included: impeccable's engine and its detector rules, live mode, `generate`, hooks, `doctor`, scripts, the standalone shell, and the `init` and `document` commands, because `DESIGN.md` in MoxyWolf repositories is written by `/gstack-design-doc`. See `NOTICE` and `licenses/impeccable-Apache-2.0.txt`.
