> Modified by MoxyWolf LLC (2026-09-22) from impeccable v4.3.1 at `e0881d2` (Apache-2.0, (c) 2025 Paul Bakaus): the native-platform links point at the platform guidelines instead of references this plugin doesn't carry, and the hand-off points at `/polish`. See the plugin `NOTICE`.
...
- **Native (`ios` / `android` / `adaptive`):** follow the platform's own motion guidance (Apple Human Interface Guidelines, Material Design), including its Reduce Motion behavior. Do not apply the web tooling below.
