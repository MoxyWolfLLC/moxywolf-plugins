> Modified by MoxyWolf LLC (2026-09-22) from impeccable v4.3.1 at `e0881d2` (Apache-2.0, (c) 2025 Paul Bakaus): the sentence about impeccable's editor hook is replaced, because this plugin ships no hook. See the plugin `NOTICE`.

# Craft floor

Load this after the direction is settled, and build without announcing the checklist. A pinned brief or the committed visual world overrides anything here; your own habit does not. No hook enforces these here, so run the checks yourself.
