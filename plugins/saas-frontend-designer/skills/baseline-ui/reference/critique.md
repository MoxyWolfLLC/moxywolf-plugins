...
- Assessment A (design review) and Assessment B (mechanical sweep and browser evidence) are both required.
...
2. **Derive the slug** from the resolved target: lowercase it and replace every run of characters outside `a-z0-9` with a single `-`, trimming `-` from both ends. A vague or root-level target has no slug; skip persistence and trend for that run, but continue the critique.
3. **Read `.critique/ignore.md`** if it exists. Drop matching findings silently; it is the only prior-run input critique consumes.
...
Evaluate:
- **Design specificity**: Is the composition, interaction, and visual language grounded in this product, or could an unrelated product use it unchanged? Make this judgment before seeing sweep output.
...
### Assessment B: Mechanical Sweep + Browser Evidence

Sweep the target against fixed rules, not taste. Assessment B is mandatory and must remain isolated from Assessment A until both are complete.

Source sweep:
- Read the target's markup, styles and components. For a directory, list the files you will read first; for very large trees (500+ files), narrow scope or ask.
- Check every item under **Refuse** in [craft-floor.md](craft-floor.md) and record each hit with file and line.
- Check the mechanical items under **Verify** in [craft-floor.md](craft-floor.md) that can be read from source: contrast pairs you can compute from declared colors, type measure and scale, missing states.
- Check the Absolute bans in this skill's `SKILL.md`.
- For URLs with no source, skip the source sweep and say so.

Browser evidence, when browser automation is available and the target is viewable:
1. Create a fresh tab and navigate. Use a localhost dev or static URL for local files; avoid `file://` unless the browser explicitly supports it.
2. Capture desktop and mobile widths in one batch.
3. Check what the source could not show: computed contrast, overflow at each width, focus visibility, and the browser surfaces craft-floor names.
4. For multi-view targets, sample 3-5 representative pages.

Return: findings with rule name, file and line (or URL and element), counts per rule, the list of files and pages examined, likely false positives, and any skipped browser steps with concrete reasons.

### Generate Combined Critique Report

Synthesize both assessments into a single report. Do NOT simply concatenate. Weave the findings together, noting where the design review and the sweep agree, where the sweep caught issues the review missed, and where sweep findings are false positives.
...
**Mechanical sweep**: Summarize what the sweep found, with counts, file locations and the files examined. Note issues the sweep caught that the review missed, and flag false positives.
...
- **Suggested playbook**: Which playbook could address this (the matching playbook in `reference/`: animate, audit, bolder, clarify, colorize, critique, distill, harden, interaction-design, layout, onboard, quieter, typeset)

#### Persona Red Flags
> *Consult the [Personas reference](#persona-based-design-testing) below.*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If the project has a `PRODUCT.md`, or `CLAUDE.md` contains a `## Design Context` section, also generate 1-2 project-specific personas from the audience/brand info.
...
Do this first because the alternative is the most common way this command fails: the report gets composed once, straight into the persistence heredoc, and the run ends with a perfect archive nobody has read. Composing it into a file is not delivering it. If the report exists only in `.critique/`, the run produced nothing.
...
1. **Write the snapshot** to `.critique/<slug>-<YYYYMMDD-HHMM>.md`. Start it with a frontmatter block carrying `target` (the user's phrasing), `resolved` (the path or URL), `total_score`, `max_score` (the applicable maximum from the heuristic table, 40 when every heuristic applied), `na_heuristics` (comma-separated numbers, or empty), `p0_count` and `p1_count`. The body is the full critique report (heuristic table, design-specificity verdict, priority issues, persona red flags, minor observations, and questions), stopping before the "Ask the User" and "Recommended Actions" sections.
...
- Only recommend the matching playbook in `reference/`: animate, audit, bolder, clarify, colorize, critique, distill, harden, interaction-design, layout, onboard, quieter, typeset; plus `/polish` and `/a11y-audit`
...
>
> Run the critique again after fixes to see your score improve.
...
If the project has a `PRODUCT.md`, or `CLAUDE.md` contains a `## Design Context` section, derive 1–2 additional personas from the audience and brand information:
