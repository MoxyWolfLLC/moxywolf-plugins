...
- Call `mcp__claude-code-remote__list_triggers` and look for scheduled tasks already registered by this plugin. Match on name — `Commitment calendar` and `Morning brief`. If one exists, note its id, cron, and enabled state.
...
- what times the two briefings should arrive, in local time
