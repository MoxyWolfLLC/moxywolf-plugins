...
2. **Read the staged blog** at `<piece>/04-diligence/blog.md` to identify its central metaphor (one sentence). Use the post's title, the angle from `01-delegation.md`, and the first 200 words of body to extract it.
3. **Compose the hero prompt inline.** Template:

    ```
    A {style_keywords} composition representing {central_metaphor}.
    Palette restricted to {palette}.
    {aspect_ratio} aspect ratio, {dimensions}.
    Forbidden: {forbidden}. No watermarks. No borders.
    ```

   Substitute the values from the brand block.

4. **Show the prompt to the user via AskUserQuestion or as a markdown block, and ask for explicit approval BEFORE invoking image generation.** The whitepaper's Diligence ethos requires this transparency. The user can accept, edit, or reject.

5. **On approval, call whatever image-generation MCP the user has connected** (Krea, HuggingFace Spaces, Gemini, etc.). The plugin doesn't pin to one provider — it surfaces the prompt, the user approves, and you pick the available tool. If none is available, surface the prompt as a markdown file at `<piece>/04-diligence/og-hero-prompt.md` and tell the user to generate the image by hand and drop the PNG at `<piece>/04-diligence/og-hero.png`.

6. **Save artifacts.** The PNG goes to `<piece>/04-diligence/og-hero.png`. The prompt (including which tool generated the image, or "(generated by hand)") goes to `<piece>/04-diligence/og-hero-prompt.md` for the AI-transparency audit trail.
