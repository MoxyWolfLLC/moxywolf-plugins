...
8. **Snapshot the raw draft.** `cp <piece>/03-discernment/draft.md <piece>/03-discernment/draft.raw.md` — before any rewriting. This is the only record of what Claude actually sampled; step 9 edits `draft.md` in place and the baseline is unrecoverable afterwards. `/blog-rewrite` and `scripts/perturbation.py` both need it.
9. **Rewrite against findings.** Edit the draft to address every Major and Medium finding. Do not convert "Not X. Not Y. Z." into "X, Y, and Z" — that is punctuation laundering, and it changes the marks rather than the cadence. Do not trade a three-beat reveal for a two-beat contrast; that moves the count between columns and changes nothing on the page.
10. **Second-pass audit (mandatory).** Re-run Layer 1 + Layer 2 on the rewrite. Any surviving Tier-2-Major means the rewrite was cosmetic — redo the whole composition step, not patch. **If the second-pass score is worse than the first, stop patching and restart from outline**; a rising score means the findings are being satisfied literally rather than structurally. Outputs `slop-findings-pass2.md`.
11. **Letter grade** — A or B advances to Phase 4. C forces re-rewrite. D or F aborts the phase and demands a structural rethink.
12. **Any override gets written down.** A long-form piece may legitimately ship over a Layer-2 objection on a declared signature device. Record the Layer-1 grade, the Layer-2 counts, the density numbers, and who signed, in `slop-findings.md`. A quiet skip is not an override.
