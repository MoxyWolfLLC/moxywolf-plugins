...
**What it does:**
1. **Voice load (mandatory STEP 0).** Read the writer's voice profile — `<blog-project-dir>/<author-slug>-voice.md`, located by walking up from CWD to find `blog-project-instructions.md`, reading its `Author` field, and resolving the sibling voice file. Read in full. Report back to the user what was loaded (voice tone, sentence-length range, contraction rate target, fragment frequency, conjunction-starter frequency, forbidden phrases). Per jamon8888/cc-suite — never proceed silently. If the voice profile is missing, halt and point the user at `/4d-blog-engine:blog-voice` to create one. The plugin never applies a preset voice.
1b. **Phase 1 → Phase 2 carry (v0.1.1+).** Scan the Phase 1 earned-secret + audience persona for content matching the 8 voice-injection slots. Pre-fill any slots that are substantively answered. Present the pre-filled set to the user for confirmation in a single message. Only re-ask the unanswered slots in step 2. This avoids wasting user turns when the earned secret already supplies Trigger / Evidence / Emotional Core, which it often does.
2. **Voice interview (carried slots skipped)** — reuse `research-pipeline/content-writer`'s interview (Trigger / Evidence / Contrarian Take / Authority / Specific Reader / Business Connection / Call to Action / Emotional Core). One question per message for the un-carried slots only. Push back on vague answers ("Marketing professionals" is not a reader; "Sarah, VP Marketing at a Series B SaaS company who just got told to use AI more by her CEO" is a reader).
