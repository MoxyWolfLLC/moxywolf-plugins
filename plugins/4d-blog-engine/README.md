...
**Pillars (hub-and-spoke).** The engine asks **new pillar or existing pillar** on every post — there is no orphan path. A pillar is a real canonical hub *page* (e.g. `stigviewer.com/methodology`) carrying richer schema than a post (`Article`/`TechArticle` + `FAQPage` + `Organization`). Each pillar has a versioned **linking map** — committed in the target's `linking_map_dir`, scaffolded from `references/linking-map-template.md` — that tracks the hub, the spoke inventory, on-site internal links, hub→spoke "Related reading," and anchor-text guidance. A spoke links its first mention of the hub term up to the hub — **auto-wired at build time** by the shared **`@moxywolf/hub-links`** package, which every target's `auto_linker` now points at (`rehype`/`remark`/`html`/`react` adapter per site). The cross-property term map `hub-links/src/map.ts` is the single source of truth for what auto-links where (a term owned by one property links from all of them — relative on the owner, absolute elsewhere; see DR-079); because linking is build-time, posts only *mention* the term — they don't hand-insert the first-mention link. `/blog-publish` registers each new spoke in the linking map and holds the live spoke→hub link until the hub page is live (never linking to a 404); the hub maintains its down-links; everything updates in one pass. Manage pillars with `/blog-pillar`, and the shared term map with **`/blog-term`** (`/blog-pillar` registers a new pillar's term there automatically). The methodology is generalized from `Taskade/STIGViewer/12 – MARCOM/methodology-hub-and-spoke-linking-map-2026-06-14.md` and codified in DR-079.
