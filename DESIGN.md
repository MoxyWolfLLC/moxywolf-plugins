...
3. The refusal does not fire for a reviewer that has no filesystem. The `api` transports already record `read_tracking: not_applicable` and are untouched. `_SELFTEST` uses a fake reviewer that is a shell command printing JSON and has no filesystem either, so it is exempt for the same reason and the existing selftest assertions keep their outcomes.
4. A test executes the guard rather than asserting over a hand-built dict: it drives `cmd_round` with a fake reviewer whose output is well formed and whose surface is untouched, and asserts the outcome is `examined_nothing` and not `malformed_output`. A test that only checked the condition in isolation would repeat XE-014 criterion 10's defect at a smaller scale.
